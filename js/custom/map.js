...
function showWelcome() {
    ...
    let dist = getDistance(31.85008,117.260807, ipLocation.data.lng, ipLocation.data.lat); //改为自己的经纬度，写法121.413921（经度lng）,31.089290（纬度lat）
    ...
    try {
        ...
        document.getElementById("welcome-info").innerHTML =
            `欢迎来自 <b><span style="color: var(--icedream-ip-color);font-size: var(--icedream-gl-size)">${pos}</span></b> 的小友<br>${posdesc}<br>当前位置距博主约 <b><span style="color: var(--icedream-ip-color)">${dist}</span></b> 公里！<br>您的IP地址为：<b><span>${ip}</span></b><br> ${timeChange}<br>`;
    } catch (err) {
